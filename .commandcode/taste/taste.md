...
- Prefers mobile layouts to preserve desktop design patterns (e.g., centered section headers with flanking lines, centered single-item grids) over left-aligned mobile simplifications. Confidence: 0.85
- Prefers search bar with fixed consistent size that never obscures brand/logo text across focused/unfocused states over expanding/resizing focus behavior. Confidence: 0.9
- Prefers interactive swipe transitions that track the finger live with slide + cross-fade (current view fades/slides out while target fades in, gallery-style) over snap-on-release view switches. Confidence: 0.8
